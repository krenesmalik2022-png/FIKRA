...
// ── DATA & STATE ──────────────────────────────────
const MOCK_DATA = [
    { id: "FK-1092", idea: "What if there was a barber shop where you pay with content — post a review, get a free cut.", user: "@zah_creates", city: "Riyadh", remixes: 142, energy: 9800, tag: "Business", picked: false },
    { id: "FK-2281", idea: "An app that translates silence. You record dead air and it tells you what the vibe was.", user: "@moody_dev", city: "Cairo", remixes: 87, energy: 6400, tag: "Tech", picked: true },
    { id: "FK-7740", idea: "Youth-run underground radio that broadcasts from random rooftops every Friday night.", user: "@streetfreq", city: "Amman", remixes: 310, energy: 15200, tag: "Culture", picked: false },
    { id: "FK-0032", idea: "A sneaker brand that drops 1 pair per city per week. Exclusivity is the whole product.", user: "@drip_theory", city: "Dubai", remixes: 200, energy: 11300, tag: "Business", picked: false },
    { id: "FK-8812", idea: "Mental health check-ins disguised as daily mood playlists, auto-generated from your listening.", user: "@freq_mind", city: "Cairo", remixes: 415, energy: 21000, tag: "Wellness", picked: true },
    { id: "FK-D1", idea: "Solar-powered water synthesisers for arid regions.", user: "@desert_tech", city: "Dubai", remixes: 8, energy: 1500, tag: "Future", picked: true },
    { id: "FK-D2", idea: "Decentralized logistics for rural agriculture co-ops.", user: "@farm_chain", city: "Casablanca", remixes: 15, energy: 3100, tag: "Tech", picked: false }
];
...
function siteNav(screenId, navEl) {
    ...
    // Update nav links
    document.querySelectorAll('.fikra-nav-link').forEach(l => l.classList.remove('active'));
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.ms-tag-opt').forEach(btn => {
        btn.onclick = () => {
            ...
            triggerGlitch(150);
            showToast(`PROTOCOL: ${activeTag ? activeTag.toUpperCase() : 'UNKNOWN'}_ENGAGED`);
        };
    });
    ...
    document.getElementById('remix-submit-btn')?.addEventListener('click', handleRemixSubmit);
...
function resetDropForm() {
    ...
    document.querySelectorAll('.ms-check-item').forEach(el => el.classList.remove('complete'));
    document.querySelectorAll('.ms-tag-opt').forEach(b => b.classList.remove('active'));
...
function renderFeed() {
    ...
    if (filtered.length === 0) {
        container.innerHTML = '<div class="empty-state">No ideas here yet. Be the first!</div>';
        return;
    }
...
function updateFeedStats() {
    ...
    const map = {
        ...
        'stat-remixes': totalRemixes >= 1000 ? (totalRemixes / 1000).toFixed(1) + 'k' : totalRemixes,
        'stat-cities': cities,
        'stat-energy': totalEnergy >= 1000 ? Math.round(totalEnergy / 1000) + 'k' : totalEnergy
    };
...
function remixIdea(postId) {
    ...
    if (badge) badge.innerHTML = `<span class="op-label">REMIXING</span> <span class="op-handle">${post.user}</span>`;
    ...
    if (treeEl) {
        if (post.remixChain?.length > 0) {
            treeEl.innerHTML = post.remixChain.map(r => `
...
function handleDropSubmit(e) {
    ...
    const input = document.getElementById('idea-input');
    const submitBtn = document.getElementById('submit-btn-v3');
    if (!input || input.value.trim().length < 5 || !activeTag || !submitBtn) return;
...
function populateProfileScreen() {
    ...
    const bio = profile.bio || 'Unfiltered. Original. Building the future one fikra at a time.';
    ...
    const totalEnergy = appFeedData.reduce((s, p) => s + (p.energy || 0), 0);
    const myRemixes = appFeedData.reduce((s, p) => s + (p.remixChain?.filter(r => r.user === '@me').length || 0), 0);
    ...
    appFeedData.forEach(p => {
        if (p.remixChain) {
            p.remixChain.forEach(r => allRemixes.push({ ...r, tag: p.tag, origUser: p.user }));
        }
    });
...
function calcRank(energy, drops) {
    ...
    if (energy >= 20000 || drops >= 10) return { label: 'Visionary', emoji: '🚀' };
...
function buildActivityGrid() {
    ...
    for (let i = 0; i < 364; i++) {
        ...
        const level = seed < 5 ? 0 : seed < 7 ? 1 : seed < 9 ? 2 : seed < 10 ? 3 : 4;
...
function shareProfile() {
    ...
    const name = profile.name || 'Zaid Al-Harbi';
    if (navigator.share) {
        navigator.share({ title: `${name} on FIKRA`, text: `Check out ${name}'s ideas on FIKRA!`, url: window.location.href });
    } else {
...
function saveProfile() {
    ...
    loadProfile();
    closeProfileModal();
    if (document.getElementById('profile-screen')?.classList.contains('active')) populateProfileScreen();
...
function closeProfileModal() {
    ...
    setTimeout(() => { if (backdrop) backdrop.style.display = ''; }, 300);
}
...
function observeCards() {
    document.querySelectorAll('.card:not(.reveal)').forEach(card => cardObserver?.observe(card));
}
...
function buildEnergyChart(data) {
    ...
    const eLine = points.map((p, i) => `${i === 0 ? 'M' : 'L'} ${ex(i)} ${ey(p.energy)}`).join(' ');
    const eFill = eLine + ` L ${ex(days - 1)} ${H} L ${ex(0)} ${H} Z`;

    const rLine = points.map((p, i) => `${i === 0 ? 'M' : 'L'} ${ex(i)} ${ry(p.remixes)}`).join(' ');
    const rFill = rLine + ` L ${ex(days - 1)} ${H} L ${ex(0)} ${H} Z`;

    const setPath = (id, d) => { const el = document.getElementById(id); if (el) el.setAttribute('d', d); };
...
const TAG_COLORS = {
    Tech: '#00D2FF', Future: '#8A3FFF', Business: '#FFD700',
    Culture: '#FF6B6B', Wellness: '#00C97E', Education: '#FF8C00', Community: '#BB8FCE'
};

function buildTopIdeas(data) {
    const container = document.getElementById('di-top-ideas');
    if (!container) return;
    const sorted = [...data].sort((a, b) => (b.energy + b.remixes * 50) - (a.energy + a.remixes * 50)).slice(0, 6);
...
const EVENT_TEMPLATES = [
    (p) => ({ icon: '�', msg: `INTEL ACQUIRED [${p.city.toUpperCase()}]: "${p.idea.slice(0, 35)}..."` }),
    (p) => ({ icon: '�️', msg: `REINFORCEMENTS: ${p.remixes} supports detected on op "${p.idea.slice(0, 25)}"` }),
    (p) => ({ icon: '⚡', msg: `POWER SURGE: ${formatEnergy(p.energy)} energy units deployed to #${p.tag.toUpperCase()}` }),
    (p) => ({ icon: '🎯', msg: `SECTOR SECURED: ${p.city} engagement levels critical` }),
    (p) => ({ icon: '⚠️', msg: `HIGH PRIORITY: "${p.idea.slice(0, 30)}..." gaining friction` }),
];
...
function buildLiveFeed(data) {
    ...
    feed.innerHTML = shuffled.map((p, i) => {
...
function startLiveTicker(data) {
    ...
    liveTickerInterval = setInterval(() => {
        ...
        const tmpl = EVENT_TEMPLATES[Math.floor(Math.random() * EVENT_TEMPLATES.length)](p);
...
function buildVibeArc(energy) {
    ...
    if (arc) {
        ...
        arc.style.transition = 'stroke-dashoffset 1.5s cubic-bezier(0.165,0.84,0.44,1)';
    }
    const vibes = energy > 60000 ? 'LEGENDARY' : energy > 30000 ? 'ELECTRIC' : energy > 10000 ? 'RISING' : 'WARMING';
...
function buildTagBars(data) {
    ...
    if (!container) return;
    const counts = {};
    data.forEach(p => { counts[p.tag] = (counts[p.tag] || 0) + 1; });
...
function buildRankCard(energy, drops) {
    const rank = [...RANKS].reverse().find(r => energy >= r.min || drops >= r.min / 1000) || RANKS[0];
    ...
    if (next) next.textContent = rank.next === 'MAX' ? '👑 Maximum Rank!' : `${100 - rank.pct}% to ${rank.next}`;
    if (prog) setTimeout(() => { prog.style.width = rank.pct + '%'; }, 300);
}
...
function animateRemixNetwork() {
    if (!document.getElementById('remix-screen').classList.contains('active')) return;
    if (!remixCtx) return;

    remixCtx.clearRect(0, 0, remixCtx.canvas.width, remixCtx.canvas.height);

    // Draw connections between nodes
    const elements = document.querySelectorAll('.remix-node, .original-idea-preview');
    const rects = Array.from(elements).map(el => el.getBoundingClientRect());
    const screenRect = document.getElementById('remix-screen').getBoundingClientRect();

    remixCtx.strokeStyle = document.body.classList.contains('light-theme') ? 'rgba(138, 63, 255, 0.1)' : 'rgba(138, 63, 255, 0.2)';
...
function handleTerminalCommand(e) {
    ...
    function addLine(text, type = 'default') {
        ...
        line.style.color = type === 'error' ? '#FF3B3B' : type === 'success' ? '#FFB000' : '#00FF41';
...
function updateMissionUI() {
    ...
    missionProgress = Math.min(ACTIVE_MISSION.target, Math.floor(totalRemixes / 2));
